...
> **На Windows інсталятор самодостатній.** Ядро аналізу (заморожений
> PyInstaller-збіркою Python-воркер) лежить усередині — ні Python, ні PATH, ні
> жодної команди ставити не треба. Перший аналіз один раз завантажує
> vision-модель (~1 ГБ) і кешує її; далі все працює офлайн. FFmpeg для відео —
> один клік у самому застосунку.
>
> **macOS і Linux ще не самодостатні:** там потрібен Python 3.11+ зі
> встановленим FileSight (див. [Встановлення](#встановлення-windows-powershell)).
> Заморожування ядра під ці платформи — наступний крок: PyInstaller не вміє
> крос-компілювати, а збирати macOS-x64 через Rosetta без можливості
> перевірити результат на живому залізі — гірше, ніж не робити зовсім.
...
Інсталятор для Windows містить заморожене ядро, тому спершу треба зібрати
воркер — інакше Tauri впаде на відсутньому ресурсі:

```powershell
pip install ".[build]"
python scripts/build-worker.py     # ~7 хв, ~615 МБ у src-tauri/resources/
python scripts/verify-worker.py    # доводить, що воно СПРАЦЮЄ, а не лише зібралось
```

`verify-worker.py` — не формальність: збірка, яка успішно злінкувалась, може
не вміти captionити взагалі, і видно це лише в рантаймі. Скрипт говорить
справжнім протоколом по справжніх пайпах і вантажить справжню модель.

Далі власне інсталятор:
